...
if torch is not None:
    ...
    class ToyChannelsLastClassifier(nn.Module):
        ...
        def forward(self, inputs: torch.Tensor) -> torch.Tensor:
            features = self.target(inputs.permute(0, 2, 3, 1))
            return self.head(features.mean(dim=(1, 2)))
    ...
    class FakeSwin(nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.features = nn.Sequential(nn.Identity(), nn.Sequential(FakeBlock()))
...
@unittest.skipUnless(torch is not None, "PyTorch is required for Grad-CAM tests.")
class SwinGradCAMTests(unittest.TestCase):
    def make_input(self) -> torch.Tensor:
        inputs = torch.zeros(1, 3, 4, 5)
        inputs[:, 0] = torch.arange(1, 21, dtype=torch.float32).reshape(4, 5)
        inputs[:, 1] = torch.arange(1, 21, dtype=torch.float32).reshape(4, 5) * 0.25
        return inputs

    def test_predicted_class_heatmap_is_normalized(self) -> None:
        model = ToyChannelsLastClassifier()
        with SwinGradCAM(model, model.target) as explainer:
            result = explainer.generate(self.make_input())

        self.assertEqual(result.predicted_class, 0)
        self.assertEqual(result.target_class, 0)
        self.assertEqual(tuple(result.heatmap.shape), (4, 5))
        ...
        self.assertEqual(len(model.target._forward_hooks), 0)

    def test_explicit_target_can_differ_from_prediction(self) -> None:
        model = ToyChannelsLastClassifier()
        with SwinGradCAM(model, model.target) as explainer:
            result = explainer.generate(self.make_input(), target_class=1)
    ...
    def test_invalid_target_is_rejected(self) -> None:
        ...
        with SwinGradCAM(model, model.target) as explainer:
            with self.assertRaises(ValueError):
                explainer.generate(self.make_input(), target_class=2)

    def test_swin_layer_resolution_supports_wrappers(self) -> None:
        model = WrappedFakeSwin()
        self.assertIs(unwrap_swin(model), model.network)
        self.assertIs(
            resolve_swin_target_layer(model, "last_block_norm1"),
            model.network.features[-1][-1].norm1,
        )
        self.assertIs(
            resolve_swin_target_layer(model, "last_block_norm2"),
            model.network.features[-1][-1].norm2,
        )
        self.assertIs(
            resolve_swin_target_layer(model, "final_norm"),
            model.network.norm,
        )
