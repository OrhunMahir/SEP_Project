...
animal, it passes the unchanged raw image to Swin. This is the same fallback
used during final training and inference.
...
PYTHONPATH=src python scripts/gradcam_swin.py \
  ...
  --dataset-root /path/to/dataset/all \
  --limit 10 \
  --target-class true \
...
`summary.json` records every requested image, including raw-fallback cases.
