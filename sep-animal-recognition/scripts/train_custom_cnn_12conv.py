#!/usr/bin/env python3
"""Train the separate twelve-convolution custom CNN from random initialization."""
...
class ThreeConvBlock(nn.Module):
    """Three Conv-BN-ReLU layers followed by one spatial downsampling operation."""
...
class CustomCNN12Conv(nn.Module):
    """Twelve-convolution CNN using four three-convolution feature blocks."""
    ...
    def __init__(self, num_outputs: int = NUM_OUTPUTS, dropout: float = 0.3) -> None:
        ...
        self.features = nn.Sequential(
            ThreeConvBlock(3, 32),
            ThreeConvBlock(32, 64),
            ThreeConvBlock(64, 128),
            ThreeConvBlock(128, 256),
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "custom_cnn_12conv.json",
    )
    ...
    if model_config["name"] != "custom_cnn_12conv":
        raise ValueError("This script only supports model.name='custom_cnn_12conv'.")
    ...
    print("Architecture: 12 convolution layers (3 per feature block)")
    ...
    summary = {
        ...
        "architecture": "custom_cnn_12conv",
        "device": str(device),
