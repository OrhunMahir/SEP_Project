#!/usr/bin/env python3
"""Train an eight-convolution custom CNN with Squeeze-and-Excitation blocks."""
...
class SqueezeExcitation(nn.Module):
    """Lightweight channel attention for convolutional feature maps."""
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        weights = self.excitation(self.pool(inputs)).view(inputs.shape[0], inputs.shape[1], 1, 1)
        return inputs * weights


class TwoConvSEBlock(nn.Module):
    """Two Conv-BN-ReLU layers, SE channel attention, then downsampling."""

    def __init__(self, in_channels: int, out_channels: int, se_reduction: int) -> None:
        super().__init__()
        self.layers = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            SqueezeExcitation(out_channels, reduction=se_reduction),
...
class CustomCNN8ConvSE(nn.Module):
    """Eight-convolution CNN using SE attention in each feature block."""
    ...
    def __init__(
        ...
        self.features = nn.Sequential(
            TwoConvSEBlock(3, 32, se_reduction),
            TwoConvSEBlock(32, 64, se_reduction),
            TwoConvSEBlock(64, 128, se_reduction),
            TwoConvSEBlock(128, 256, se_reduction),
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "custom_cnn_8conv_se_medium_aug.json",
    )
    ...
    if model_config["name"] != "custom_cnn_8conv_se":
        raise ValueError("This script only supports model.name='custom_cnn_8conv_se'.")
    ...
    print("Architecture: 8 convolution layers with SE blocks (2 convs per feature block)")
    print("Augmentation: medium crop, rotation, color jitter, perspective, grayscale, erasing")
    ...
    for epoch_index in range(max_epochs):
        ...
        if selection_score > best_selection_score:
            ...
            torch.save(
                {
                    "model_name": "custom_cnn_8conv_se",
    ...
    summary = {
        ...
        "architecture": "custom_cnn_8conv_se",
        "augmentation": "medium",
        "se_reduction": se_reduction,
        "device": str(device),
