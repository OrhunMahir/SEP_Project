...
def main() -> None:
    ...
    with SwinGradCAM(
        ...
        for index, item in enumerate(
            ...
            if detector is not None:
                ...
                # Match the final inference pipeline: a detector miss does not
                # force reject. The unchanged image is passed to the classifier.
                if crop_box is not None:
    ...
    summary = {
        ...
        "raw_fallbacks": sum(
            record.get("preprocessing") == "raw_fallback"
            for record in records
        ),
        "records": records,
    }
