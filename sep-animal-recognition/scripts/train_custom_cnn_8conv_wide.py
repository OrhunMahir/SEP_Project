#!/usr/bin/env python3
"""Train a wider eight-convolution custom CNN from random initialization."""
...
class TwoConvBlock(nn.Module):
    """Two Conv-BN-ReLU layers followed by one spatial downsampling operation."""
...
class CustomCNN8ConvWide(nn.Module):
    """Eight-convolution CNN with wider feature channels."""

    def __init__(
        self,
        num_outputs: int = NUM_OUTPUTS,
        dropout: float = 0.3,
        channels: list[int] | tuple[int, int, int, int] = (48, 96, 192, 384),
    ) -> None:
        super().__init__()
        ...
        if len(channels) != 4:
            raise ValueError("channels must contain exactly four values.")
        if any(channel <= 0 for channel in channels):
            raise ValueError("all channels must be positive.")

        channel_1, channel_2, channel_3, channel_4 = [int(channel) for channel in channels]
        self.features = nn.Sequential(
            TwoConvBlock(3, channel_1),
            TwoConvBlock(channel_1, channel_2),
            TwoConvBlock(channel_2, channel_3),
            TwoConvBlock(channel_3, channel_4),
        )
        self.global_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.dropout = nn.Dropout(dropout)
        self.classifier = nn.Linear(channel_4, num_outputs)
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "custom_cnn_8conv_wide_medium_aug.json",
    )
    ...
    if model_config["name"] != "custom_cnn_8conv_wide":
        raise ValueError("This script only supports model.name='custom_cnn_8conv_wide'.")
    ...
    print("Architecture: wider 8 convolution layers (2 per feature block)")
    ...
    for epoch_index in range(max_epochs):
        ...
        if selection_score > best_selection_score:
            ...
            torch.save(
                {
                    "model_name": "custom_cnn_8conv_wide",
    ...
    summary = {
        ...
        "architecture": "custom_cnn_8conv_wide",
        "channels": channels,
        "augmentation": "medium",
        "device": str(device),
