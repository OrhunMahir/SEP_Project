#!/usr/bin/env python3
"""Train the separate eight-convolution custom CNN from random initialization."""
...
class CustomCNN8Conv(nn.Module):
    """Eight-convolution CNN using four two-convolution feature blocks."""

    def __init__(self, num_outputs: int = NUM_OUTPUTS, dropout: float = 0.3) -> None:
        super().__init__()
        if num_outputs != NUM_OUTPUTS:
            raise ValueError(
                f"CustomCNN8Conv requires {NUM_OUTPUTS} outputs, received {num_outputs}."
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "custom_cnn_8conv.json",
    )
    ...
    print("Architecture: 8 convolution layers (2 per feature block)")
